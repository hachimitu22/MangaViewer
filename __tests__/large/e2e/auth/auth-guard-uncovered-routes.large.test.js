...
describe('large e2e: 認可境界（未カバー導線）', () => {
  ...
  test('未ログインでは未カバー保護 API が 401 になり、ログイン後は同一 API が許可される', async () => {
    ...
    const unauthorizedResults = await page.evaluate(async ({ baseUrl, mediaId, postContentId }) => {
    ...
    }, { baseUrl, mediaId: detailMediaId, postContentId: contentIdForPost });
    ...
    const authorizedResults = await page.evaluate(async ({ baseUrl, mediaId, postContentId }) => {
      const postFormData = new FormData();
