...
const parseListPaths = value => (value || '').split(',').map(entry => entry.trim()).filter(entry => entry.length > 0);
...
const resolveServerHost = (source = {}) => {
  ...
  return isProduction(source.NODE_ENV) ? '0.0.0.0' : '127.0.0.1';
};
...
const createEnv = source => ({
  ...
  allowedHosts: parseListPaths(source.APP_ALLOWED_HOSTS || '127.0.0.1,localhost,::1'),
  ...
  logTargets: source.LOG_OUTPUTS || (source.NODE_ENV === 'test' ? 'memory' : 'console,file'),
});
