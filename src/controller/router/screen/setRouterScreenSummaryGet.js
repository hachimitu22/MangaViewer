...
const normalizeTags = rawTags => (rawTags === undefined ? [] : (Array.isArray(rawTags) ? rawTags : [rawTags]))
  ...
  .map(value => {
    ...
    return { category: value.slice(0, separatorIndex).trim(), label: value.slice(separatorIndex + 1).trim() };
  })
...
const normalizeSearchRange = ({ summaryPage, start, size }) => {
  ...
  if (normalizedStart !== null) {
    return { summaryPage: Math.floor((normalizedStart - 1) / normalizedSize) + 1, start: normalizedStart, size: normalizedSize };
  }
  const normalizedSummaryPage = normalizePositiveInteger(summaryPage, DEFAULT_SUMMARY_PAGE);
  return { summaryPage: normalizedSummaryPage, start: ((normalizedSummaryPage - 1) * normalizedSize) + 1, size: normalizedSize };
};
...
const createPagination = ({ totalCount, summaryPage, pageSize }) => {
  ...
  for (let page = 1; page <= totalPages; page += 1) {
    ...
    if (isEdge || isNearCurrent || totalPages <= 7) pages.push(page);
  }
  const items = [];
  pages.forEach(page => {
    if (items.length > 0 && page - items[items.length - 1] > 1) items.push('ellipsis');
    items.push(page);
  });
