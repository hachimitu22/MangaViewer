...
- `ImportZipsService` はユースケース結果種別（`SUCCESS` / `PARTIAL_FAILURE` / `INVALID_INPUT` / `INVALID_INPUT_RUN_ZIP_COUNT_LIMIT_EXCEEDED`）までを返し、数値終了コードへの変換責務は CLI が担う。
- 入力契約（引数不足、フォルダ不存在、権限不足、上限値不正など）の検証は `ImportZipsService` が担い、CLI は返却された結果種別を終了コードへ変換する。
- `0`: 正常終了（対象 zip がすべて成功、または対象 zip が 0 件）。
- `1`: 処理実行は完了したが、1件以上の zip が失敗。
- `2`: 入力契約エラー（引数不正、フォルダ不存在、権限不足、上限値不正など）により未実行終了。
