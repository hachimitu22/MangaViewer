...
## テストケース一覧
- [GET /screen/edit/:mediaId に描画ハンドラーを登録する](#get-screeneditmediaid-に描画ハンドラーを登録する)
- [登録済みハンドラーを実行すると編集画面を描画する](#登録済みハンドラーを実行すると編集画面を描画する)
- [詳細取得で例外が発生した場合は next に委譲する](#詳細取得で例外が発生した場合は-next-に委譲する)
- [編集テンプレートの削除導線は確認後にのみ DELETE を実行する](#編集テンプレートの削除導線は確認後にのみ-delete-を実行する)

---

### GET /screen/edit/:mediaId に描画ハンドラーを登録する
- **前提**
  - `router.get` をモック化する。
